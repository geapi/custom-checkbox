describe("custom_checkbox", function() {
	...
	describe("clicking on the checkbox", function() {
		it("should uncheck a checked checkbox", function() {
			$parent = $(".styled_checkbox_wrap #checked");
			spyOn(jQuery.fn, "parents").andReturn($parent);

			var removeClassSpy = spyOn($parent, "removeClass");
			uncheckedCheckbox.trigger("click");
			expect(removeClassSpy).toHaveBeenCalledWith("wrap_checked");
		});

		it("should check an uncheck checkbox", function() {
			$parent = $(".styled_checkbox_wrap #unchecked");
			spyOn(jQuery.fn, "parents").andReturn($parent);

			var addClassSpy = spyOn($parent, "addClass");
			checkedCheckbox.trigger("click");
			expect(addClassSpy).toHaveBeenCalledWith("wrap_checked");
		});
